...
# MAGIC %md
# MAGIC # Bronze Layer — Card Details Ingestion
# MAGIC **Source:** Card Management System | **Format:** Parquet | **Target:** credit_card_dev.bronze.card_details
...
logger = PipelineLogger(spark, "bronze_card_details_ingest", run_id)
# Track incremental load watermark
wm = Watermark(spark, CATALOG)
last_ts = wm.get("bronze_card_details")
print("Watermark [bronze_card_details]: last_processed_ts = " + str(last_ts))
...
source_path = f"{LANDING_VOLUME}/card/card_details"
checkpoint_path = f"{CHECKPOINT_BASE}/card_details"
target_table = BRONZE_CARD_DETAILS
...
reader = (
    ...
    .option("badRecordsPath", f"{LANDING_VOLUME}/_bad_records/card_details")
...
wm.update("bronze_card_details", new_ts=datetime.utcnow().isoformat(), run_id=run_id)
