...
# MAGIC %md
# MAGIC # Bronze Layer — CRM Customer Address Ingestion
# MAGIC **Source:** CRM System | **Format:** CSV | **Target:** credit_card_dev.bronze.crm_customer_address
...
logger = PipelineLogger(spark, "bronze_crm_address_ingest", run_id)
# Track incremental load watermark
wm = Watermark(spark, CATALOG)
last_ts = wm.get("bronze_crm_address")
print("Watermark [bronze_crm_address]: last_processed_ts = " + str(last_ts))
...
source_path = f"{LANDING_VOLUME}/crm/customer_address"
checkpoint_path = f"{CHECKPOINT_BASE}/crm_customer_address"
target_table = BRONZE_CRM_ADDRESS
...
reader = (
    ...
    .option("badRecordsPath", f"{LANDING_VOLUME}/_bad_records/crm_address")
    .schema(CRM_ADDRESS_SCHEMA)
    .load(source_path)
)
...
wm.update("bronze_crm_address", new_ts=datetime.utcnow().isoformat(), run_id=run_id)
